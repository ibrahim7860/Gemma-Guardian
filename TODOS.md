...
### Convert `test_e2e_phase3.py::test_e2e_reconnect_after_bridge_restart` from stub to live test
- **What:** The reconnect-after-bridge-restart e2e at `frontend/ws_bridge/tests/test_e2e_phase3.py:274` is currently a `pytest.skip(...)` stub. Convert it into a working test that (a) drives a fresh-finding round trip, (b) kills + restarts the bridge subprocess, (c) reconnects the Flutter dashboard, and (d) asserts the previously-approved finding still surfaces with `operator_status == "approved"` (i.e., the bridge stamp survives a restart via `egs.state.approved_findings`). This was originally Task 6 sub-deliverable C of the 2026-05-11 plan but got deferred since the target test was a skip stub — extending "one assertion" required writing the whole test, which was out of PR scope.
- **Why:** Catches a regression class the new green-check e2e doesn't reach: the bridge aggregator's stamp depends on `egs_state` being current at snapshot time, but on bridge restart the aggregator's egs bucket re-seeds from a fixture envelope until the next 1 Hz `egs.state` publish lands. If EGS-side state is stale at the moment the freshly-reconnected dashboard renders, the row appears as pending. The unit `test_snapshot_handles_missing_or_none_approved_findings_field` test covers the partial case but not the full restart-cycle path.
