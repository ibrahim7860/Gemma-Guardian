...
### Multi-drone integrated C2A path on 8 GB-class GPUs
- **What:** `scripts/run_drone3_reliability.sh` (F1 reliability harness) launches all three resilience_v1 drone agents in parallel. Each drone agent unconditionally instantiates a `C2AInferenceNode` in-process via `agents/drone_agent/runtime.py:_try_load_c2a` whenever the default adapter path (`kaggle_work_c2a/adapter/`) exists — which it does in any checkout (the stub `qasim_inference.py` / `run_sard_batch.py` live there). Three parallel cold loads of `unsloth/gemma-4-E2B-it` 4-bit do not fit in 8 GB of VRAM and the disk/CPU thrash also delays the agent main loop past the 240 s scripted-event window where the victim frame is published. Net effect: drone3 doesn't see the victim frame until after the sim has self-terminated, so the harness reports `drone3 did not emit any report_finding` even though the C2A load + inference path itself works fine. Reproduced 2026-05-16 on RTX 3060 Ti / 8 GB / WSL2 Ubuntu 24.04, run dir `/tmp/gg_drone3_reliability_1778951673/`.
- **Why:** Two compounding issues. (1) **`env -u C2A_ADAPTER_PATH` is insufficient** to disable C2A on a specific drone — the argparse default at `agents/drone_agent/__main__.py:64` falls back to the in-tree `kaggle_work_c2a/adapter/` path, which exists even when only stubs are present. (2) **No `--no-c2a` CLI flag** exists on the drone agent, so the test harness can't selectively opt out per-process.
- **Pros:** Adding a `--no-c2a` flag (or treating `--c2a-adapter-path=""` as opt-out) lets the F1 reliability harness load C2A only on the drone that needs it (drone3 sees the victim frame; drone1/drone2 don't). Single-C2A runs fit comfortably on 8 GB. Useful both for low-VRAM CI/dev boxes and for keeping the demo-capture run cheap.
- **Cons:** Requires a small drone-agent CLI change (Kaleel's lane) plus a paired script edit in `scripts/run_drone3_reliability.sh` (Hazim's lane) — both are easy but neither touches a load-bearing test path. Deliberately deferred past the May 18 submission lock.
- **Context:** F1 was originally on the Sat-AM checklist (`docs/plans/2026-05-15-remaining-work-to-submission.md` §F1) and got deferred 2026-05-16 to Qasim's RTX A2000 8 GB box on the grounds that Qasim already proved 3/3 integrated victim detection on his hardware (STATUS L51, commit `962fc28`) for a *single-drone* invocation. The multi-drone integrated path with C2A on all three drones has never been validated end-to-end on any hardware, so this TODO captures the gap.
- **Owner:** TBD post-submission (CLI flag → Kaleel; harness wiring → Hazim).
