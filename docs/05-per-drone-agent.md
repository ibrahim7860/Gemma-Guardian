...
CLI: `python -m agents.drone_agent --drone-id drone1 --scenario disaster_zone_v1`. Defaults source from `shared/config.yaml` via `shared.contracts.config.CONFIG`. The mission-zone bbox is bootstrapped at startup from the scenario YAML (mission-wide, all drones' waypoints + 50m, via the shared `shared.contracts.zones.mission_zone_bbox` helper). At runtime, an `EgsStateSubscriber` consumes `egs.state.zone_polygon` and overwrites the bootstrap on every valid envelope — the EGS is the single source of truth for the survey area. Closed by `docs/plans/2026-05-13-drone-zone-from-egs-state.md`.
