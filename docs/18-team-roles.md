...
|---|---|---|---|
| Hazim | Simulation Lead | `sim/waypoint_runner.py`, `sim/frame_server.py`, `sim/scenarios/*.yaml`, `agents/mesh_simulator/main.py`, Redis infra, launch scripts | Gate trajectory call at standup, integration testing leadership |
| Kaleel | Per-Drone Agent + ML | Drone LangGraph agent, function calling, validation loop, **xBD fine-tuning, vision prompts, adapter integration** | Prompt iteration across the full drone-side stack |
| Qasim | EGS / Coordination | EGS LangGraph, swarm allocation, multilingual command path | Scripting resilience scenarios |
| Ibrahim | The Elevator Watcher — Frontend + Demo + Comms | Flutter dashboard, FastAPI WebSocket bridge, demo capture, **video editing, technical writeup, README**, **overall project management** | Map view, multilingual UI, demo storyline, scope-cut arbitration |
| Thayyil | Simulation Co-Pilot (paired with Hazim) | Disaster scene frame library curation, ground-truth manifest, scenario YAML authoring, launch scripts — every sim/infra task that can be parallelized | All integration-testing prep work for Hazim |
...
**Interfaces with:**
- Kaleel: provides `drones.<id>.camera` and `drones.<id>.state` on Redis; provides frames from `sim/fixtures/frames/` for vision prompt iteration (via Thayyil)
- Qasim: provides `drones.<id>.state` for telemetry
- Ibrahim: the FastAPI WebSocket bridge (`frontend/ws_bridge/`) connects to the same Redis; Hazim confirms Redis is reachable from all processes
- **Thayyil (dedicated co-pilot):** works alongside Hazim every day on the entire sim stack — frame library curation, scenario authoring, launch scripts, integration testing. Thayyil is paired exclusively with Hazim.

**This is the highest-risk single role, which is why Thayyil is dedicated to it.** The previous stack (Gazebo + PX4 + ROS 2) had the most install-hell risk. The new Python/Redis stack significantly lowers that risk, but Hazim still owns the most cross-component plumbing. Pairing Thayyil with Hazim effectively turns this seat into a two-person team, doubling parallelism on the workstream most likely to block everyone else.
...
**Day 7 output:** Full single-drone agentic loop running end-to-end with Hazim's drone. LoRA training in progress on xBD.
...
**Interfaces with:**
- Hazim: subscribes to `drones.<id>.camera` and `drones.<id>.state` on Redis; receives frames from `sim/fixtures/frames/` for vision prompt iteration (Thayyil curates these)
- Qasim: publishes findings and state for EGS to aggregate

**This role is by far the heaviest single seat on the team.** Kaleel needs strong familiarity with LangGraph, prompt engineering, Python async, *and* PyTorch / Hugging Face / Unsloth fine-tuning. They are a single point of failure for both the agent loop and the ML pipeline; the Day-10 NO-GO gate is non-negotiable insurance. **Thayyil is dedicated to Hazim, so Kaleel has no shared support resource** — if Kaleel falls behind on the agent because of fine-tuning, the team must invoke the NO-GO gate early rather than reassign help.
...
**Day 7 output:** Real drone telemetry from Hazim / Kaleel's drones drives real assignments. Validation loop catches engineered failures.
...
**Interfaces with:**
- Hazim: receives `drones.<id>.state` from Redis (published by `sim/waypoint_runner.py` and merged with drone agent fields)
- Kaleel: sends task assignments to drone agents via `drones.<id>.tasks` on Redis
- Ibrahim: pushes shared state to Flutter via the FastAPI WebSocket bridge (`egs.state` on Redis → `ws://localhost:9090`); receives operator commands; provides EGS context for the writeup

Qasim owns resilience scenario scripting alone — Thayyil is dedicated to Hazim and not available for cross-team support.

**This role is the second most agentic-AI heavy.** Qasim owns the wow moment: the validation loop catching a hallucination on stage. They must engineer the demo scenario where this reliably happens.

## Ibrahim: Frontend + Demo + Comms
...
**Interfaces with:**
- Qasim: receives state updates via `ws://localhost:9090` and sends operator commands; receives EGS technical context for the writeup
- Hazim (via Thayyil): receives the ground-truth manifest for dashboard data

**This role merges what was previously Ibrahim (Frontend) and Thayyil (ML/Demo/Comms), minus the ML workstream which moved to Kaleel.** The merge works because the timing splits cleanly: Days 1-13 are frontend-heavy buildout, Days 14-20 are demo-and-comms heavy. The dashboard is largely feature-complete by Gate 4, freeing the same person to capture, edit, and write. Ibrahim must be a strong web engineer with solid technical writing skills and basic video editing comfort. **Thayyil is dedicated to Hazim, so Ibrahim has no shared support resource** — if frontend slips past Day 7, the team must descope rather than reassign help.

## Thayyil: Simulation Co-Pilot (paired exclusively with Hazim)

**Thayyil works exclusively with Hazim.** They are not a floater across the team. Every day, Thayyil sits inside the simulation workstream and parallelizes whatever Hazim is doing. This pairing exists because Hazim's seat owns the most cross-component plumbing, and the sim/infra workstream has the most parallelism-friendly tasks (frame curation, scenario YAML authoring, launch-script plumbing, integration testing).

**Owns end-to-end (always in collaboration with Hazim):**
...
- **Mesh dropout simulator support** — helping Hazim wire `agents/mesh_simulator/main.py`
- **Resilience scenario scripting** for the demo (drone failures, comm dropouts) — encoded as scripted events in the scenario YAML, owned within the Hazim / Thayyil workstream
- **Integration testing prep** — building the test harnesses and check scripts that Hazim uses to verify everyone's code works together
- **Reproduction documentation for the simulation stack** — they are the natural "tester who didn't write it"

**Day 1-2 output:** Initial frame library curated from xBD dataset. xBD download handled (on Hazim's machine — no further xBD data-prep work; Kaleel owns that). Scenario YAML skeleton drafted.
**Day 7 output:** Scenario v1 complete with frame mappings for all waypoints. Ground-truth manifest drafted.
**Day 10 output:** Scenario v2 complete (victims, fires, debris frames). Multi-drone scenario YAML working with Hazim.
...
**Interfaces with:**
- **Hazim:** every day, all day. This is a true pairing.
- Other teammates: only via Hazim. Thayyil does not directly support Kaleel, Qasim, or Ibrahim. If those roles need help, the answer is descope, not reassignment.

**Why exclusive pairing:** Hazim's stack owns the most cross-component plumbing and has the most parallelizable work (frame curation, YAML authoring, launch scripting, testing). Splitting Thayyil across multiple roles dilutes that effort and leaves Hazim still solo on the riskiest seat. By dedicating Thayyil to Hazim, we effectively make the simulation workstream a two-person team. The cost: Kaleel, Qasim, and Ibrahim have no shared support resource, which means descope-not-reassignment is the team's mitigation strategy if those roles slip.
...
Standup ends with Hazim (Simulation Lead) reporting gate trajectory ("We're on track for Gate N at Day X") and Ibrahim (The Elevator Watcher) confirming or calling for descope. Hazim also confirms Redis + sim processes are stable before declaring the day green.
...
- `sim/<feature>` (Hazim and Thayyil — they share the namespace since they pair on this stack)
- `agent/<feature>`, `ml/<feature>` (Kaleel)
- `egs/<feature>` (Qasim)
- `frontend/<feature>`, `comms/<feature>` (Ibrahim)
- `scene/<feature>`, `mesh/<feature>` (Thayyil specifically, when isolating their work)
...
| Demo storyline | Ibrahim proposes, team approves |
| Scope cuts (final call after team vote) | Ibrahim (The Elevator Watcher) |
| Submission timing | Ibrahim (The Elevator Watcher) calls it (Day 18 by default), with Hazim confirming sim-stack readiness |
...
- [ ] At least one person comfortable with Python scripting + Redis + scripted automation (Hazim) — any modern OS with Python 3.11+, `redis-server`, and Ollama
- [ ] A second person comfortable with Python scripting and YAML config, willing to pair daily on simulation work (Thayyil — does not need ML, frontend, LangGraph, or ROS 2 / Gazebo experience)
- [ ] At least one person comfortable with Python async + LangGraph **AND** PyTorch / Hugging Face / Unsloth (Kaleel — both skill sets required in the same seat). **Comfortable provisioning a cloud GPU instance** (Lambda Labs / Paperspace / Runpod) if no local NVIDIA GPU is available.
- [ ] At least one person comfortable with Python async + LangGraph (Qasim) — any OS with Ollama
- [ ] At least one strong web engineer (Flutter / web frontend) **who can also handle technical writing and basic video editing** (Ibrahim — frontend + comms merged) — any OS
