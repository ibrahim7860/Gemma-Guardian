...
| F8 | post-mission | The verifier runs: `uv run python scripts/check_beat5.py --bridge-url ws://127.0.0.1:9090 --validation-log /tmp/gg_beat5_capture/validation_events.jsonl --deadline-s 30`. All six A-assertions PASS in green. | A1 standalone entry, A2 finding while standalone, A3 delivery only after restore, A4 EGS count tick within 5 s, A5 return to active, A6 no double-count under replay. See `docs/plans/2026-05-10-beat5-path-a-full.md` §9. |
