...
**Left (GATE 2 critical, today/tomorrow):** Wire drone agent to real Redis camera + state channels (currently runs from local fixtures); publish findings on `drones.<id>.findings` per Contract 4; wire validation-loop log to `/tmp/fieldagent_logs/validation_events.jsonl` end-to-end. Spec: [`docs/05-per-drone-agent.md`](05-per-drone-agent.md), [`docs/10-validation-and-retry-loop.md`](10-validation-and-retry-loop.md).
