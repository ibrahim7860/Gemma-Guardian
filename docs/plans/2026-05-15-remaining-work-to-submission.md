...
- [x] **B1.** ~~Run GATE 4 evaluation~~ → **RE-VOTE 2026-05-17: PASS on 3-drone.** Ibrahim re-ran the full resilience_v1 scenario against a cloud-hosted RTX 3090 24GB (RunPod, $0.22/hr Community), pointing `shared/config.yaml` `ollama_drone_endpoint` / `ollama_egs_endpoint` at the HTTPS proxy. Both Gemma 4 models resident in VRAM simultaneously (19GB / 24GB) — zero swap, zero ReadTimeout. Full 240s timeline ran to completion, 70 validation events, 0 crashes. Per-drone throughput soft-failed (0.08 Hz vs 0.5 Hz target) but environmental, not architectural. Full re-vote results in `docs/decisions.md` "GATE 4 RE-VOTE — 2026-05-17".
- [x] **B2.** ~~Decision recorded in `docs/decisions.md`~~ → **Updated 2026-05-17:** Re-vote addendum appended. 2-drone fallback rescinded. Ollama-mock-server mitigation rescinded. Demo proceeds with 3-drone configuration as storyboarded.
- [x] **B3.** ~~If FAIL → drop to 2 drones~~ → **REVERSED 2026-05-17:** Stay on 3-drone configuration. Cloud-Ollama path validated end-to-end.
