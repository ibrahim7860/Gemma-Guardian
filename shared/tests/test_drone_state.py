"""Contract 2 (drone_state) schema round-trip."""
...
@pytest.mark.parametrize(
    "fixture",
    sorted((FIXTURES / "valid" / "drone_state").glob("*.json")),
    ids=lambda p: p.name,
)
def test_valid(fixture):
    outcome = validate("drone_state", _load(fixture))
    assert outcome.valid is True, outcome.errors


@pytest.mark.parametrize(
    "fixture",
    sorted((FIXTURES / "invalid" / "drone_state").glob("*.json")),
    ids=lambda p: p.name,
)
