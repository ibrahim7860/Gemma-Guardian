...
- **`drone_failure → drones.<id>.tasks` chain: ✅ unblocked 2026-05-12** by Ibrahim's GH #32 fix (took the open ticket off Qasim's lane). Both halves landed in one PR: Bug 2 (`replanning.py` re-raised httpx errors instead of falling back) + Bug 3 (`_replan_impl` could hang and lock `_replan_in_flight=True` forever). Fix: catch `httpx.HTTPError`/`asyncio.TimeoutError`/`json.JSONDecodeError` as retryable + wrap the assign_survey_points await in `asyncio.wait_for(REPLAN_OVERALL_TIMEOUT_S=240s)`. Live re-run on a VRAM-constrained box still pending. The original Bug 1 (egs_state schema seed) closed via PR #34/#45 commit `9cda8cb` — verified live (0 schema errors vs 268 on 2026-05-07).
